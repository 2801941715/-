...
1. **合法合规使用**：本工具仅用于学习 Appium 移动端自动化技术，请勿用于任何商业用途或违反服务条款的行为
2. **风险自负**：使用本工具可能存在账号被封禁、订单异常等风险，使用者需自行承担所有后果
3. **尊重平台规则**：请严格遵守大麦网的用户协议和服务条款，不得进行恶意刷票或影响平台正常运营的行为
4. **技术研究目的**：本项目主要用于研究移动端自动化技术，不鼓励大规模或商业化使用
...
- **一键式操作** - 简单易用的图形界面
- **图形化配置** - 可视化填写城市、票价、观演人等参数
- **设备与服务自检** - 自动检测 Appium 服务与 adb 设备状态
- **定时抢票** - 可预约开抢时间并设置预热检查
- **实时日志显示** - 清晰显示抢票进度和状态
...
- **Python 3.7+**
- **Android 真机/模拟器**（开启 USB 调试）
- **核心依赖库**：
  - `Appium-Python-Client`
  - `pydantic`
  - `selenium`（Appium 运行器的传递依赖）
  - `tkinter`（Python 标准库）
