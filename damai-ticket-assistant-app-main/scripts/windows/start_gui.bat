...
python -c "import appium" >nul 2>&1
if %errorlevel% neq 0 (
    echo ⚠ Missing appium library, installing automatically...
    ...
    if %errorlevel% neq 0 (
        ...
        echo    pip install Appium-Python-Client
        echo.
        pause
        exit /b 1
    )
...
if %errorlevel% neq 0 (
    ...
    echo 3. Android device / ADB not available
    echo.
    echo Solutions:
    echo 1. Install dependencies manually: pip install -r requirements.txt
    echo 2. Install Android Platform Tools (adb)
